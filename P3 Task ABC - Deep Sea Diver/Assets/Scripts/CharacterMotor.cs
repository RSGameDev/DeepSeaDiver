...
public class CharacterMotor : MonoBehaviour
{
    ...
    public class CharacterMotorMovement
    {
        ...
        public float maxAirAcceleration = 3.0f;
    ...
    void FixedUpdate()
    {
        ...
        transform.eulerAngles = new Vector3(0, transform.eulerAngles.y + (Input.GetAxis("Horizontal") * rotateSpeed), 0);
        speed = transform.InverseTransformDirection(movement.velocity).z;
        if (IsJumping())
        {
            animationModel.GetComponent<Animation>().Blend("Jet Loop");
        }
        else if (speed > 0.1 || speed < -0.1)
        {
            animationModel.GetComponent<Animation>().Blend("Run");
            if (speed < -0.1)
            {
                animationModel.GetComponent<Animation>()["Run"].speed = -1;
            }
            else
            {
                animationModel.GetComponent<Animation>()["Run"].speed = 1;
        ...
        {
            //animationModel.animation.Blend("Idle");
            animationModel.GetComponent<Animation>().CrossFade("Idle");
        }
        if (useFixedUpdate)
            UpdateFunction();
    }
    ...
    private Vector3 ApplyGravityAndJumping(Vector3 velocity)
    {
        ...
        if (grounded)
        {
            ...
            if (jumping.enabled && canControl && (Time.time - jumping.lastButtonDownTime < 0.2f))
            {
                ...
                animationModel.GetComponent<Animation>().Play("Jet Loop");
